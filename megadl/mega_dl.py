...
async def megadl(bot, message):
    ...
    try:
      if 'folder' in url:
        await message.reply_text(f"**Sorry,mega folder Download is not supported yet!**", reply_to_message_id=message.message_id)
        return
      else:
